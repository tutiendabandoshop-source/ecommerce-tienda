...
import { DollarSign, Package, TrendingDown, Plus, BarChart3, RotateCcw } from 'lucide-react';
import { resetContabilidad } from './actions';
...
export default function ContabilidadClient() {
  ...
  // Opciones: para TODOS los productos siempre hay opción "Original" (producto base); si tiene variantes, se añaden además
  const variantOptions = useMemo((): VariantOption[] => {
    const opts: VariantOption[] = [];
    for (const p of products) {
      // Siempre incluir opción del producto base (Original) → usa productId y stock de Product
      opts.push({
        value: `product:${p.id}`,
        productId: p.id,
        productName: p.name,
      ...
      // Si tiene variantes, añadir una opción por cada una → usan variantId y stock de ProductVariant
      const variants = p.variants ?? [];
      for (const v of variants) {
        const label = [v.color, v.size].filter(Boolean).join(' / ') || 'Variante';
        opts.push({
          value: v.id,
          productId: p.id,
          productName: p.name,
  ...
  const filteredVariantOptions = useMemo(() => {
    return variantOptions.filter((opt) => {
      const matchSearch = !productSearch || opt.productName.toLowerCase().includes(productSearch.toLowerCase());
      const product = products.find((p) => p.id === opt.productId);
      const matchCat = !categoryFilter || (product?.categoryId === categoryFilter);
      return matchSearch && matchCat;
    });
  }, [variantOptions, productSearch, categoryFilter, products]);

  const selectedVariant = useMemo(
    () => variantOptions.find((o) => o.value === selectedVariantId),
  ...
  const isCatalogSale = !useExternal && selectedVariantId && !showAddExternal;
  ...
  const handleResetContabilidad = async () => {
    const confirmed = confirm(
      '¿Estás seguro? Se borrará todo el historial financiero y pedidos, pero el STOCK de productos no se verá afectado.'
    );
    if (!confirmed) return;
    setResetLoading(true);
    const result = await resetContabilidad();
    ...
    if (result.success) {
      ...
      alert(result.error ?? 'Error al resetear contabilidad');
    }
  };
  ...
  const handleAddSale = async (e: React.FormEvent) => {
    ...
    if (useExternal) {
      ...
      if (selectedVariantId.startsWith('product:')) {
        productId = selectedVariantId.slice(8);
      } else {
        variantId = selectedVariantId;
        productId = selectedVariant?.productId ?? null;
      }
    }
                ...
                {!useExternal ? (
                  <>
                    ...
                    >
                      ...
                      {filteredVariantOptions.map((opt) => (
                        <option key={opt.value} value={opt.value}>
                          {opt.productName} ({opt.variantLabel}) - ${opt.price.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </option>
                      ))}
                    </select>
                    <button
                  ...
                  <tbody>
                    {sales.map((s) => (
                      <tr key={s.id} className="border-b border-gray-100">
                        ...
                        <td className="py-2 px-4 border-b border-gray-100">
                          {s.clientName ? (parseClientName(s.clientName).name?.trim() || '-') : 'Anónimo'}
                        </td>
