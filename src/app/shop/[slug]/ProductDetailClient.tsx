...
export default function ProductDetailClient({ product }: { product: Product }) {
  ...
  // Disponibilidad efectiva: si hay variante con stock > 0, es "stock listo"; si no, la del producto padre
  ...
  const showParentAvailability = !selectedVariant || selectedVariant.stock <= 0;
            ...
            {hasVariantWithStock ? (
              currentStock <= 5 ? (
                <div className="inline-flex items-center bg-warning/20 text-secondary-dark px-4 py-2 rounded-xl font-bold">
                  Stock listo · ¡Solo quedan {currentStock}!
                </div>
              ) : (
                <div className="inline-flex items-center gap-2 bg-action/20 text-action-dark px-4 py-2 rounded-xl font-bold">
                  <span className="w-2 h-2 bg-action rounded-full" />
                  Stock listo
                </div>
              )
            ) : showParentAvailability ? (
              product.isPreOrder ? (
